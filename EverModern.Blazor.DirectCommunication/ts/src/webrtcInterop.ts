...
function addLocalTracks(id: string): void {
  ...
  stream.getTracks().forEach((track) => {
    ...
    if (existingSender) {
      ...
      if (connection.localDescription || connection.remoteDescription) {
        warn(
          id,
          `Adding ${track.kind} track after SDP negotiation. A renegotiation may be required for remote audio to work.`
        );
      }
