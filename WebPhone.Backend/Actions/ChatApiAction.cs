    ...
    : ApiActionConcrete<SendChatInput, ChatMessageDto>
{
    ...
    {
        var payload = JsonSerializer.SerializeToElement(new { text = input.Request.Text }, JsonOptions);
        var sentAt = DateTime.UtcNow;
        var messageId = CommonIdsGenerator.NewId();

        await repository.WriteMessagesAsync(
        [
            new MessageWriteEntry(ChatMessageType, payload, input.ClientId, input.Request.RecipientId, sentAt, messageId)
        ], cancellationToken: cancellationToken);

        var dto = new ChatMessageDto(messageId, input.ClientId, input.Request.RecipientId, input.Request.Text, sentAt);

        var userSettings = await userSettingsRepository.GetAsync(input.Request.RecipientId, cancellationToken);
        var contactSettings = await contactSettingsRepository.GetAsync(input.Request.RecipientId, input.ClientId, cancellationToken);
        ...
        if (shouldNotify)
        {
            ...
            _ = pushNotificationService.PushToClientAsync(input.Request.RecipientId, pushPayload, cancellationToken);
        }

        logger.LogInformation("[CHAT] {From} → {To}: {Preview}, push={Push}",
            input.ClientId, input.Request.RecipientId, input.Request.Text[..Math.Min(input.Request.Text.Length, 60)], shouldNotify);

        return dto;
    }
}

public sealed class GetChatMessagesApiAction(MessagesRepository repository)
